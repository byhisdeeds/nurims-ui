...
let slateReact = require('slate-react');
// let slateHistory = require('slate-history');
let React = require('react');
let React__default = _interopDefault(React);
let PropTypes = _interopDefault(require('prop-types'));
let Box = _interopDefault(require('@mui/material/Box'));
let styles = require('@mui/styles');
let core = require('@mui/material');
let IconButton = _interopDefault(require('@mui/material/IconButton'));
let Tooltip = _interopDefault(require('@mui/material/Tooltip'));
let SvgIcon = _interopDefault(require('@mui/material/SvgIcon'));
let ReactDOM = _interopDefault(require('react-dom'));
let Button = _interopDefault(require('@mui/material/Button'));
let TextField = _interopDefault(require('@mui/material/TextField'));
let Dialog = _interopDefault(require('@mui/material/Dialog'));
let DialogTitle = _interopDefault(require('@mui/material/DialogTitle'));
let DialogContent = _interopDefault(require('@mui/material/DialogContent'));
let DialogActions = _interopDefault(require('@mui/material/DialogActions'));
let Typography = _interopDefault(require('@mui/material/Typography'));
...
let MaterialEditor = _extends({}, slate.Editor);
 ...
 * @param {Editor} editor
 */
let withBase = function withBase(editor) {
  /**
  ...
  editor.isNodeTypeActive = function (type) {
    let _MaterialEditor$nodes = MaterialEditor.nodes(editor, { match: function match(n) {
          return n.type === type;
        } }),
  ...
  editor.wrapNode = function (node) {
    let wrapSelection = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    ...
    let path = [].concat(toConsumableArray(MaterialEditor.last(editor, editor.selection)[1]));
  ...
  editor.syncExternalNodes = function (type, nodesToKeep) {
    let unwrap = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;

    //extracts the id from the nodes and removes those that are not in the list
    let listOfIds = nodesToKeep.map(function (node) {
      return node.id;
    });
    ...
    let editorNodes = editor.findNodesByType(type);
    //Update them
  ...
  editor.findNodesByType = function (type) {
    let list = MaterialEditor.nodes(editor, {
      match: function match(n) {
        return n.type === type;
      },
      at: []
    ...
    let listWithNodesAndPath = Array.from(list);
    // List with node (element)
    let listWithNodes = listWithNodesAndPath.map(function (item) {
      return item[0];
    });
  ...
  editor.syncExternalNodesWithTemporaryId = function (type, nodesToKeep) {
    ...
    editorNodes.map(function (node) {
      ...
      let key = Object.keys(nodesToKeepObj).find(function (key) {
        return nodesToKeepObj[key].temporaryId === node.id;
      });

      // node to Update with original Id and data
      let nodeToUpdate = nodesToKeepObj[key];
      // If node.id exists
 ...
 * @param {Editor} editor
 */
let withMarks = function withMarks(editor) {
  ...
  editor.isMarkActive = function (mark) {
    let marks = MaterialEditor.marks(editor);
 ...
 * @param {Editor} editor
 */
let withBlocks = function withBlocks(editor) {
  ...
  editor.isBlockActive = function (block) {
    let _MaterialEditor$nodes = MaterialEditor.nodes(editor, {
        match: function match(n) {
          return n.type === block;
        }
      }),
  ...
  editor.toggleBlock = function (block) {
    let isActive = editor.isBlockActive(block);
    let isList = editor.LIST_TYPES.includes(block);
    ...
    if (!isActive && isList) {
      let selected = { type: block, children: [] };
...
function createMaterialEditor() {
  let editorId = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'default';

  // let editor = withBlocks(withMarks(withBase(slateHistory.withHistory(slateReact.withReact(slate.createEditor())))));
  let editor = withBlocks(withMarks(withBase(slateReact.withReact(slate.createEditor()))));
  editor.editorId = editorId;
  return editor;
}

let withComments = function withComments(editor) {
  let isInline = editor.isInline;
 ...
 */
let withEndnotes = function withEndnotes(editor) {
  let isInline = editor.isInline,
    isVoid = editor.isVoid;


  let ENDNOTE_TYPE = 'endnote';
  ...
  editor.previousEndnoteNode = function (endnoteId) {
    let previous = null;
    let endnotes = editor.findNodesByType(ENDNOTE_TYPE);
    let _iteratorNormalCompletion = true;
    let _didIteratorError = false;
    let _iteratorError = undefined;
    let _iterator = undefined;
    let _step = undefined;

    try {
      for (_iterator = endnotes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        let endnote = _step.value;
 ...
 * @param {Editor} editor
 */
let withCounter = function withCounter(editor) {
  /**
  ...
  editor.getWordsLength = function (nodes) {
    let content = editor.serialize(nodes);
...
let withLinks = function withLinks(editor) {
  let isInline = editor.isInline;
  ...
  editor.insertLink = function (url) {
    ...
    let wrapSelection = editor.selection || editor.rememberedSelection;
    editor.selection = wrapSelection ? wrapSelection : editor.selection;
    let node = {
      type: LINK_TYPE,
      url: url,
...
let useStyles = styles.makeStyles(function (theme) {
  return {
    root: {
...
let lib = createCommonjsModule(function (module, exports) {
  ...
  let IS_MAC = typeof window != 'undefined' && /Mac|iPod|iPhone|iPad/.test(window.navigator.platform);
  ...
  function isHotkey(hotkey, options, event) {
    ...
    let array = hotkey.map(function (string) {
      return parseHotkey(string, options);
    });
    let check = function check(e) {
      return array.some(function (object) {
        return compareHotkey(object, e);
      });
    };
    let ret = event == null ? check : check(event);
    return ret;
  }
  ...
  function parseHotkey(hotkey, options) {
    let byKey = options && options.byKey;
    let ret = {};

    // Special case to handle the `+` key since we use it as a separator.
    hotkey = hotkey.replace('++', '+add');
    let values = hotkey.split('+');
    ...
    try {
      for (_iterator = values[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
  ...
  function toKeyCode(name) {
    name = toKeyName(name);
    let code = CODES[name] || name.toUpperCase().charCodeAt(0);
    return code;
  }
...
let isHotkey = unwrapExports(lib);
let lib_1 = lib.isHotkey;
let lib_2 = lib.isCodeHotkey;
let lib_3 = lib.isKeyHotkey;
let lib_4 = lib.parseHotkey;
let lib_5 = lib.compareHotkey;
let lib_6 = lib.toKeyCode;
let lib_7 = lib.toKeyName;
...
let useStyles$1 = styles.makeStyles(function (theme) {
  return {
    editable: {
...
function MaterialEditable(_ref) {
  let renderElement = _ref.renderElement,
  ...
  let editor = slateReact.useSlate();
  let classes = useStyles$1();
  ...
  let handleRenderElement = React.useCallback(function (props) {
    return renderElement ? renderElement(props) : defaultRenderElement(props);
  }, []);

  let handleRenderLeaf = React.useCallback(function (props) {
    return renderLeaf ? renderLeaf(props) : defaultRenderLeaf(props);
  }, []);

  let handleOnKeyDown = function handleOnKeyDown(event) {
    for (let pressedKeys in hotkeys) {
      if (isHotkey(pressedKeys, event)) {
        let hotkey = hotkeys[pressedKeys];
        //console.log(hotkey)
...
let interopRequireDefault = createCommonjsModule(function (module) {
...
let _extends_1 = createCommonjsModule(function (module) {
  function _extends() {
...
let createSvgIcon_1 = createCommonjsModule(function (module, exports) {
  ...
  let _extends2 = interopRequireDefault(_extends_1);

  let _react = interopRequireDefault(React__default);

  let _SvgIcon = interopRequireDefault(SvgIcon);

  function createSvgIcon(path, displayName) {
    let Component = _react.default.memo(_react.default.forwardRef(function (props, ref) {
...
let CropSquareOutlined = createCommonjsModule(function (module, exports) {
  ...
  let _createSvgIcon = interopRequireDefault(createSvgIcon_1);

  let _default = (0, _createSvgIcon.default)(_react.default.createElement("path", {
...
let CropSquareOutlined$1 = unwrapExports(CropSquareOutlined);
 ...
 */
let ToolbarButton = React__default.forwardRef(function (_ref, ref) {
   ...
   */
  let defaultTooltip = function defaultTooltip() {
   ...
   */
  let handleOnMouseDown = function handleOnMouseDown(event) {
    event.preventDefault();
    switch (type) {
      case 'mark':
  ...
  let checkIsActive = function checkIsActive() {
    if (isActive) {
      return isActive();
    }
   ...
   */
  let isDisabled = function isDisabled() {
    let disabled = false;
...
let FormatBold = createCommonjsModule(function (module, exports) {
...
let FormatBold$1 = unwrapExports(FormatBold);
...
let BoldButton = React__default.forwardRef(function (props, ref) {
...
let FormatItalicOutlined = createCommonjsModule(function (module, exports) {
...
let FormatItalicOutlined$1 = unwrapExports(FormatItalicOutlined);
...
let ItalicButton = React__default.forwardRef(function (props, ref) {
...
let FormatUnderlined = createCommonjsModule(function (module, exports) {
...
let FormatUnderlined$1 = unwrapExports(FormatUnderlined);
 ...
 * @see ToolbarButton
 */
let UnderlinedButton = React__default.forwardRef(function (props, ref) {
...
let StrikethroughS = createCommonjsModule(function (module, exports) {
...
let StrikethroughSIcon = unwrapExports(StrikethroughS);
...
let StrikethroughButton = React__default.forwardRef(function (props, ref) {
...
let Code = createCommonjsModule(function (module, exports) {
...
let CodeIcon = unwrapExports(Code);
...
let CodeButton = React__default.forwardRef(function (props, ref) {
...
let FormatListBulleted = createCommonjsModule(function (module, exports) {
...
let FormatListBulleted$1 = unwrapExports(FormatListBulleted);
 ...
 *
 */
let BulletedListButton = React__default.forwardRef(function (props, ref) {
...
let FormatListNumbered = createCommonjsModule(function (module, exports) {
...
let FormatListNumbered$1 = unwrapExports(FormatListNumbered);
...
let NumberedListButton = React__default.forwardRef(function (props, ref) {
...
let useStyles$2 = styles.makeStyles(function (theme) {
  return {
    toolbar: {
...
let Portal = function Portal(_ref) {
  let children = _ref.children;

  return ReactDOM.createPortal(children, document.body);
};

let useStyles$3 = styles.makeStyles(function (theme) {
  return {
    hoveringToolbar: {
      position: 'absolute',
      padding: theme.spacing(1 / 4),
...
function SimpleDialog(_ref) {
  ...
  let _useState = React.useState(defaultValue),
    _useState2 = slicedToArray(_useState, 2),
    value = _useState2[0],
  ...
  let handleOnCancel = function handleOnCancel(format) {
    onCancel();
    setValue(defaultValue);
  };
  // Calls `onSave` prop and sets the default value
  let handleOnSave = function handleOnSave(event) {
...
let AddCommentOutlined = createCommonjsModule(function (module, exports) {
...
let AddCommentOutlinedIcon = unwrapExports(AddCommentOutlined);
...
let AddCommentButton = React__default.forwardRef(function (props, ref) {
...
let CallToActionOutlined = createCommonjsModule(function (module, exports) {
...
let CallToActionOutlinedIcon = unwrapExports(CallToActionOutlined);
...
let EndnoteButton = React__default.forwardRef(function (props, ref) {
  return React__default.createElement(ToolbarButton, _extends({
...
let Link = createCommonjsModule(function (module, exports) {
...
let LinkIcon = unwrapExports(Link);
...
function LinkButton(_ref) {
  ...
  let onLinkButtonDown = function onLinkButtonDown(_ref2) {
    let editor = _ref2.editor,
  ...
  let handleDialogSave = function handleDialogSave(url) {
...
let useStyles$4 = styles.makeStyles(function (theme) {
  return {
    root: {
 ...
 *
 */
let CommentElement = function CommentElement(_ref) {
  let element = _ref.element,
...
let useStyles$5 = styles.makeStyles(function (theme) {
  return {
    root: {
      cursor: 'pointer'
    }
 ...
 */
let EndnoteElement = function EndnoteElement(_ref) {
  let element = _ref.element,
    _onClick = _ref.onClick,
    attributes = _ref.attributes,
    children = _ref.children;

  let classes = useStyles$5();
  return React__default.createElement(
    Tooltip,
...
let useStyles$6 = styles.makeStyles(function (theme) {
  return {
    text: {
...
function WordCounter(_ref) {
  let maxWords = _ref.maxWords;

  let classes = useStyles$6();
  let editor = slateReact.useSlate();
  let children = editor.children;
  // Words length

  let wordsLength = editor.getWordsLength(children);
  // Error based on words length limit
  let errorExceedWordsLimit = wordsLength > maxWords;
...
let useStyles$7 = styles.makeStyles(function (theme) {
  return {
    text: {
      marginTop: theme.spacing(0.5),
      marginLeft: theme.spacing(1)
...
function CharCounter(_ref) {
  let maxChars = _ref.maxChars;

  let classes = useStyles$7();
  let editor = slateReact.useSlate();
  let children = editor.children;
  // Char length

  let charLength = editor.getCharLength(children);
  // Error based on chars length limit
  let errorExceedCharsLimit = charLength > maxChars;
