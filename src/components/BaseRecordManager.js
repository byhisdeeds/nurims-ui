    ...
    if (messageHasResponse(message)) {
      ...
      if (messageResponseStatusOk(message)) {
        ...
        if (this.isCommand(message, [
          ...
          } else if (this.isCommand(message, [CMD_UPDATE_REACTOR_SAMPLE_IRRADIATION_AUTHORIZATION_RECORD])) {
            const item = message.response.reactor_irradiation_authorization;
            if (recordHasMetadataField(item, NURIMS_OPERATION_DATA_IRRADIATION_AUTHORIZATION_APPROVAL_DATE)) {
              enqueueSuccessSnackbar(`Successfully approved request for ${message[NURIMS_TITLE]}.`);
            } else {
              enqueueSuccessSnackbar(`Successfully withdrew approval for ${message[NURIMS_TITLE]}.`);
            }
          } else {
