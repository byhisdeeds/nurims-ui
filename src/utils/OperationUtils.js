...
export function prepareExportData(message) {
  ...
  if (dataset === "stats") {
    if (datasetFormat === "json") {
      ...
      const header = Object.keys(data_array[0]).join(",");
      // Setup values from object values
      // const values = data_array.map(item => Object.values(Array.isArray(item)? {}:item).join(","));
      // const values = data_array.reduce( (acc, val) =>
      //   acc.concat( Object.values(Array.isArray(val) ? [] : val).join(`;`) ), [] );
      const values = data_array.map(row => {
        if (row.hasOwnProperty("rod_events")) {
          console.log("ROD-EVENTS: ", row)
          return {}
        }if (row.hasOwnProperty("flux")) {
          console.log("FLUX: ", row)
          return {}
        } else {
          console.log(">>>>", row)
          return {...row}
